...
const
    ...
    { TCP states }
    TCP_STATES = 12;
    TCP_STATE: array [1 .. TCP_STATES] of String = ('CLOSED', 'LISTEN',
...
type
    LPFN_BIND = function(const ASocket: TSocket; const AName: PSockAddr;
      const ANameLength: Integer): Integer; stdcall;
    LPFN_WSAIOCTL = function(const ASocket: TSocket; dwIoControlCode: DWORD;
      ...
      : Integer; stdcall;
    LPFN_ACCEPTEX = function(AListenSocket, AAcceptSocket: TSocket;
    ...
    LPFN_CONNECTEX = function(const ASocket: TSocket; const AName: PSockAddr;
      const ANameLength: Integer; lpSendBuffer: Pointer;
      dwSendDataLength: DWORD; var lpdwBytesSent: DWORD;
      lpOverlapped: LPWSAOVERLAPPED): BOOL; stdcall;
    LPFN_DISCONNECTEX = function(const ASocket: TSocket; AOverlapped: Pointer;
      const dwFlags: DWORD; const dwReserved: DWORD): BOOL; stdcall;

type
    LPFN_GETADDRINFOW = function(NodeName: PWideChar; ServiceName: PWideChar;
      Hints: PAddrInfoW; ppResult: PPAddrInfoW): Integer; stdcall;
    LPFN_GETNAMEINFOW = function(sa: PSockAddr; salen: DWORD; host: PWideChar;
      hostlen: DWORD; serv: PWideChar; servlen: DWORD; flags: Integer)
      : Integer; stdcall;
...
type
    LPFN_GetExtendedTcpTable = function(pTcpTable: Pointer; dwSize: PDWORD;
      bOrder: BOOL; lAf: ULONG; TableClass: TCP_TABLE_CLASS; Reserved: ULONG)
      : DWORD; stdcall;
...
{ NetStat }
function EnumerateTCPConnections(out ATcpTable
  : PMIB_TCPTABLE_OWNER_PID): Boolean;
procedure grNetstat;
...
function GetAddress(ASocket: TSocket; const AName: String; const AGuid: TGuid)
  : Pointer; inline; overload;
var
    BytesSend: DWORD;
begin
    if WSAIoctl(ASocket, SIO_GET_EXTENSION_FUNCTION_POINTER, @AGuid,
      DWORD(SizeOf(TGuid)), @Result, DWORD(SizeOf(FARPROC)), PDWORD(@BytesSend),
      nil, nil) <> 0 then
        Result := nil;
end;
...
function Overload_AcceptEx(AListenSocket, AAcceptSocket: TSocket;
  ...
  lpOverlapped: POverlapped): BOOL; stdcall;
begin
    @GetAcceptExSockAddrs := GetAddress(AListenSocket, 'GetAcceptExSockaddrs',
      WSAID_GETACCEPTEXSOCKADDRS);
    @AcceptEx := GetAddress(AListenSocket, 'AcceptEx', WSAID_ACCEPTEX);
    if @AcceptEx <> nil then
        Result := AcceptEx(AListenSocket, AAcceptSocket, lpOutputBuffer,
...
function Overload_ConnectEx(const ASocket: TSocket; const AName: PSockAddr;
  const ANameLength: Integer; lpSendBuffer: Pointer; dwSendDataLength: DWORD;
...
begin
    ...
    if @ConnectEx <> nil then
        Result := ConnectEx(ASocket, AName, ANameLength, lpSendBuffer,
          dwSendDataLength, lpdwBytesSent, lpOverlapped)
    else
        Result := False;
end;

function Overload_DisconnectEx(const ASocket: TSocket; AOverlapped: Pointer;
  const dwFlags: DWORD; const dwReserved: DWORD): BOOL; stdcall;
begin
...
begin
    ...
    begin
        ...
        if HandleIPHLPAPI <> 0 then
            GetExtendedTcpTable := GetProcAddress(HandleIPHLPAPI,
              'GetExtendedTcpTable');
    end;
    if @GetExtendedTcpTable <> nil then
    begin
        Size := 0;
        if GetExtendedTcpTable(nil, @Size, False, AF_INET,
          TCP_TABLE_OWNER_PID_ALL, 0) = ERROR_INSUFFICIENT_BUFFER then
        begin
            GetMem(ATcpTable, Size);
            if GetExtendedTcpTable(ATcpTable, @Size, TRUE, AF_INET,
              TCP_TABLE_OWNER_PID_ALL, 0) <> NO_ERROR then
            begin
                LastError := GetLastError;
                Writeln(Format('Error! GetExtendedTcpTable %d %s',
    ...
    begin
        LastError := GetLastError;
        Writeln(Format('Error! GetProcAddress %d %s',
          [LastError, SysErrorMessage(LastError)]));
    end;
end;
...
begin
    ...
    begin
        try
            Writeln(Format('%-16s %-6s %-16s %-6s %s', ['Local IP', 'Port',
              'Remote IP', 'Port', 'Status']));
        ...
        for I := 1 to TCP_STATES do
            Writeln(Format('%16s %s', [TCP_STATE[I] + ' = ',
              IntToStr(Count[I])]));
    end;
    Readln;
end;
...
try
    ...
    on e: Exception do
        Writeln('Grijjy.Winsock2: exception: ' + e.ClassName + ': ' + e.Message);
end;
